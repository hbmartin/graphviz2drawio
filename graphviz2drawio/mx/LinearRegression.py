...
def covariance(points, m) -> float:
    """Calculate the covariance of a list of complex points."""
    covar = 0.0
    for point in points:
...
def coefficient_of_determination(points, y_mean, b0, b1) -> float:
    """Calculate the R**2 value for list of complex points and a linear equation."""
    sse = []
    sst = []
    for point in points:
...
def coefficients(points: list) -> list[float]:
    """Calculate the coefficients of a lin. eq. and R**2 for a list of complex points.
    ...
    if var > 0:
        ...
        b0 = m.imag - b1 * m.real
        r2 = coefficient_of_determination(points, m.imag, b0, b1)
        return [b0, b1, r2]
